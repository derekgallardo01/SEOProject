...
def lookup_words_no_convsfn():
    with open('static/lookup_words_no_convs.csv', 'r') as file:
...
def badtook(request):
    ...
    data = pd.read_csv("static/input_data.csv", skiprows=[i for i in range(0,6)], names = header_list )
...
def convdata():
    ...
    data = pd.read_csv("static/input_data.csv", skiprows=[i for i in range(0,7)], names = header_list )
...
def convertedfound(request):
    ...
    searchConsole = pd.read_csv('static/search_console_data.csv',names = ["keyword_phrases", "clicks", "impressions", "ctr", "position"])
    ...
    vlookup_new.to_csv('static/3-converted_found_in_search_console.csv', index = False)
    ...
    not_found_in_search_console_but_converted.to_csv('static/4-converted_not_found_in_search_console.csv', index = False)
