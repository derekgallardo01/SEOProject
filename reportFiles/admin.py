...
class ReportFilesAdmin(admin.ModelAdmin):
    ...
    def merge_files(self, request, queryset):
        ...
        wb.create_sheet('Table of Contents')
        ws = wb['Table of Contents']
        ws.append(['Index', 'Sheet Name'])
        ...
        for file in books:
            ...
            filelist.append(csvdata[0].filename)

            rnn = random.randint(1,30)
            sheetname1 = csvdata[0].name
            sheetname1 = sheetname1.replace('.csv','')
            if(len(sheetname1)>26):
                sheetname1 = csvdata[0].name[:26]
                
            if(sheetname1 in sheetlist_now):
                sheetname = sheetname1+'_'+str(rnn)
            else:
                sheetname = sheetname1


            sheetlist_now.append(sheetname1)
            sheetlist.append(sheetname)
            itr +=1
            
            row = [itr,sheetname]
            
            #link = excelfilename+"#"+sheetname
            link = "#"+sheetname+"!A1"
            ws.append(row)
            ws.cell((itr+1), column=2).hyperlink = (link)
        ...
        # Save the file
        #this code for all the sheet after that are not in crawl_overview   
