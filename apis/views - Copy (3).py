...
def getfooterdata(id):
    itemdata = {}
    listdataidwise = []
    
    li = -1
    rssite = Category.objects.filter(id = int(id) ).all().order_by('id')
    rsdata = rssite[0].footer_menu_ids.split('bottomfooter:')
    firstpart = rsdata[0].replace('topfootercategoryid:','').split('||||')
    ...
    if(getidwise):
        ...
        for fid in footersite:
            ...
            listdataidwise.append({"id":fid.id,"link":link,"catname":fid.catname})
    ...
    return {'topfooter':{'1':list1, '2':list2, '3':list3, '4':list4 }}
    return chunked_data[0]
...
def footermenu(request):
    ...
    if(code=='profile' and id!=False):
        ...
        if(users and users[0].footercatid):
            rssitedetail = getfooterdata(users[0].footercatid)
    ...
    if(not rssitedetail):
        if(code=='cat' or code=='news_detail'):
            if(code=='news_detail'):
                rssite = Category.objects.filter(id = int(tab) ).all().order_by('id')
                if(rssite and rssite[0].footer_menu_ids):
                    rssitedetail = getfooterdata(rssite[0].footer_menu_ids)
                else:
                    rssite = Category.objects.filter(slug = id ).all().order_by('id')
            else:
                #rssite = Category.objects.filter(id = tab ).all().order_by('id')
                ...
                if(rssite and rssite[0].excelid):
                    ...
                    menu3 = Category.objects.filter(catname = 'footer_menu_ids').all().order_by('id')
                    rssitedetail = menu3[0].catdes.split(',')
            ...
            if(menu[0].catdes):
                ...
                if(menu1[0].top_menu_ids):
                    ...
                    menu2 = Category.objects.filter(catname = 'footer_menu_ids').all().order_by('id')
                    rssitedetail = menu2[0].catdes.split(',')
            else:
                menu3 = Category.objects.filter(catname = 'footer_menu_ids').all().order_by('id')
                rssitedetail = menu3[0].catdes.split(',')
